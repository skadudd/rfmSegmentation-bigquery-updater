create or replace table ballosodeuk.ynam.rfm_table_shopby_shoji_prop as (
    with raw as 
        (select
        ...
        ,pre_cash - 
            COALESCE(
                SUM(exchange_cash_rate) OVER (
                    ...
                    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
                , 0) as current_cash
        ,current_shoji ,coalesce(coalesce(earn,0) - coalesce(exchange,0),0) as earn, coalesce(coalesce(spend,0) - coalesce(burnt,0)) as spend, exchange, exchange_cash_rate, burnt
    from df)


    select register_dt, user_id, member_no, pre_cash, current_cash, current_shoji, earn, spend, exchange, exchange_cash_rate, burnt
    from fin_df 
    order by user_id, register_dt
)